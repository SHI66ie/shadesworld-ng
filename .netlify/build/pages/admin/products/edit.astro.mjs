...
Update Product
    ...
    <\/script>`])), maybeRenderHead(), product.name, addAttribute(product.id, "value"), addAttribute(product.name, "value"), product.description, addAttribute(product.category === "Men", "selected"), addAttribute(product.category === "Women", "selected"), addAttribute(product.category === "Unisex", "selected"), addAttribute(product.category === "Optical Frames", "selected"), addAttribute(product.sku, "value"), addAttribute(product.price, "value"), addAttribute(product.stock, "value"), addAttribute(product.image ? "block" : "hidden", "class"), addAttribute(product.image, "src"), addAttribute(`border-2 border-dashed border-zinc-700 hover:border-amber-400 bg-zinc-950 rounded-2xl p-12 text-center transition cursor-pointer flex flex-col items-center justify-center relative overflow-hidden group ${product.image ? "hidden" : ""}`, "class"), addAttribute(product.image, "value")) : renderTemplate`<div class="text-center py-16"> <p class="text-zinc-400 mb-4">Product not found</p> <a href="/admin/products" class="text-amber-400 hover:text-amber-300">← Back to Products</a> </div>`}` })}`;
